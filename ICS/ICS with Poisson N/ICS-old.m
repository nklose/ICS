...
% Last Modified by GUIDE v2.5 03-May-2012 15:29:14
...
switch handles.str{handles.val};
    ...
    case 'ICS red' % User selection
        ...
        if isempty(handles.filename) ~= 1
           ...
           for i = 1:range
            ...
            options = optimset('outputfcn',@outfun1, 'TolFun',1e-50, 'TolX', 1e-50, 'MaxFunEvals', 1000000, 'MaxIter', handles.niter);
            ...
            for mm = 1:handles.range
                newcorred1(mm) = gRnew(mm, mm);
                newcorred2(mm) = gRnew(1, mm);
                
                newcorredfit1(mm) = gRfit(mm, mm);              
                newcorredfit2(mm) = gRfit(1, mm);              
            end
            
            
            figure('Name','Autocorrelation Red X','NumberTitle','off')
            plot(delta,newcorred1,'or',...
                delta,newcorredfit1,'--r',...
                'LineWidth',2)
            
            
            figure('Name','Autocorrelation Red XY','NumberTitle','off')
            plot(delta,newcorred2,'or',...
                delta,newcorredfit2,'--r',...
                'LineWidth',2)
    ...
    case 'ICS green' % User selection
        ...
        imag = imread('Greentemp.bmp');
           ...
           for i = 1:range
            ...
            for mm = 1:handles.range
                newcorgreen1(mm) = gGnew(mm, mm);
                newcorgreen2(mm) = gGnew(1, mm);
                
                newcorgreenfit1(mm) = gGfit(mm, mm);              
                newcorgreenfit2(mm) = gGfit(1, mm);              
            end
            
            
            figure('Name','Autocorrelation Green X','NumberTitle','off')
            plot(delta,newcorgreen1,'og',...
                delta,newcorgreenfit1,'--g',...
                'LineWidth',2)
            
            
            figure('Name','Autocorrelation Green XY','NumberTitle','off')
            plot(delta,newcorgreen2,'og',...
                delta,newcorgreenfit2,'--g',...
                'LineWidth',2)
        ...
        imag = imread('Bluetemp.bmp');
           ...
           for i = 1:range
            ...
            for mm = 1:handles.range
                newcorblue1(mm) = gBnew(mm, mm);
                newcorblue2(mm) = gBnew(1, mm);
                
                newcorbluefit1(mm) = gBfit(mm, mm);              
                newcorbluefit2(mm) = gBfit(1, mm);              
            end
            
            
            figure('Name','Autocorrelation Blue X','NumberTitle','off')
            plot(delta,newcorblue1,'ob',...
                delta,newcorbluefit1,'--b',...
                'LineWidth',2)
            
            
            figure('Name','Autocorrelation Blue XY','NumberTitle','off')
            plot(delta,newcorblue2,'ob',...
                delta,newcorbluefit2,'--b',...
                'LineWidth',2)
        ...
        case 'XCS red/green' % User selection
           ...
           for i = 1:range
            ...
            for mm = 1:handles.range
                newcorrg1(mm) = gRGnew(mm, mm);
                newcorrg2(mm) = gRGnew(1, mm);
                
                newcorrgfit1(mm) = gRGfit(mm, mm);              
                newcorrgfit2(mm) = gRGfit(1, mm);              
            end
            
            
            figure('Name','Crosscorrelation Red Green X','NumberTitle','off')
            plot(delta,newcorrg1,'oy',...
                delta,newcorrgfit1,'--y',...
                'LineWidth',2)
            
            
            figure('Name','Crosscorrelation Red Green XY','NumberTitle','off')
            plot(delta,newcorrg2,'oy',...
                delta,newcorrgfit2,'--y',...
                'LineWidth',2)
        ...
        case 'XCS red/blue' % User selection
           ...
           for i = 1:range
            ...
            for mm = 1:handles.range
                newcorrb1(mm) = gRBnew(mm, mm);
                newcorrb2(mm) = gRBnew(1, mm);
                
                newcorrbfit1(mm) = gRBfit(mm, mm);              
                newcorrbfit2(mm) = gRBfit(1, mm);              
            end
            
            
            figure('Name','Crosscorrelation Red Blue X','NumberTitle','off')
            plot(delta,newcorrb1,'om',...
                delta,newcorrbfit1,'--m',...
                'LineWidth',2)
            
            
            figure('Name','Crosscorrelation Red Blue XY','NumberTitle','off')
            plot(delta,newcorrb2,'om',...
                delta,newcorrbfit2,'--m',...
                'LineWidth',2)
        ...
        case 'XCS green/blue' % User selection
           ...
           for i = 1:range
            ...
            for mm = 1:handles.range
                newcorgb1(mm) = gGBnew(mm, mm);
                newcorgb2(mm) = gGBnew(1, mm);
                
                newcorgbfit1(mm) = gGBfit(mm, mm);              
                newcorgbfit2(mm) = gGBfit(1, mm);              
            end
            
            
            figure('Name','Crosscorrelation Green Blue X','NumberTitle','off')
            plot(delta,newcorgb1,'oc',...
                delta,newcorgbfit1,'--c',...
                'LineWidth',2)
            
            
            figure('Name','Crosscorrelation Green Blue XY','NumberTitle','off')
            plot(delta,newcorgb2,'oc',...
                delta,newcorgbfit2,'--c',...
                'LineWidth',2)
        ...
        case 'Triple ICS' % User selection        
           ...
           avgB = sum(sum(B))/(SG(1)*SG(2));
            ...
            tic
                ...
                bispectdisp = real(r1(:, :, nbihlfp, nbihlfp));
                surfc(bispectdisp)
    ...
    tricorrdisp = fftshift(real(tricorrnorm(:,:,1, 1)));
    surfc(tricorrdisp)
    colormap hsv
...
%--------------------------------------------------------------------------
            ...
            TripleGfitRGB(2) = round(TripleGfitRGB(2)*correctionw*10)/10;
            set(handles.wfit, 'String', TripleGfitRGB(2));
        ...
        TripleGfitRGBforfile=0;
           ...
           if sum(sum(R)) ~= 0  && sum(sum(G)) ~= 0 && sum(sum(B)) ~= 0
            ...
            newt = ((newtricorre1+newtricorre2+newtricorre4+newtricorre5+newtricorre6+newtricorre7+newtricorre9+newtricorre10+newtricorre11+newtricorre12+newtricorre13+newtricorre14)/12)';
        ...
        %--------------------------------------------------------------------------
                    ...
                    TripleGfitRGBforfile = [TripleGfitRGB(1), TripleGfitRGB(2), TripleGfitRGB(3)];
...
%Saving fitting parameters into a file
       ...
       Title4 = ['Ginf'];
        ...
        red = cat(2, head1, num2str(AutoGfitR, '%-10.2f'));
        green = cat(2, head2, num2str(AutoGfitG, '%-10.2f'));
        blue = cat(2, head3, num2str(AutoGfitB, '%-10.2f'));
        redgreen = cat(2, head4, num2str(CrossGfitRG, '%-10.2f'));
        redblue = cat(2, head5, num2str(CrossGfitRB, '%-10.2f'));
        greenblue = cat(2, head6, num2str(CrossGfitGB, '%-10.2f'));
        redgreenblue = cat(2, head7, num2str(TripleGfitRGBforfile, '%-10.2f'));
...
stop = false;
   ...
   switch state
       ...
       case 'iter'
           ...
           curr_G0 = round(x(1)*10)/10;
           curr_w = round(x(2)*10)/10;
           curr_ginf = round(x(3)*10)/10;
           funvalue = optimValues.resnorm;
