...
def run_4(info,config):
    ...
    for i in range(6):
        if i <  3: fcode = 'AC'
        if i >= 3: fcode = 'XC'
        fpath1 = os.path.join(fdir, fprefix + fcode + ALL_COLORS[i] + '.txt')
        fpath2 = os.path.join(fdir, fprefix + fcode + ALL_COLORS[i] + 'Fit.txt')
        np.savetxt(fpath1, info.dual_out[i,:,:], fmt='%9.5f')
        np.savetxt(fpath2, info.dual_fit[i,:,:], fmt='%9.5f')
    fpath1 = os.path.join(fdir, fprefix + 'TripleCrgb.txt')
    fpath2 = os.path.join(fdir, fprefix + 'TripleCrgbFit.txt')
...
def finish(info,config):
    ...
    fpath = os.path.join(config.output_directory, 'results.txt')
