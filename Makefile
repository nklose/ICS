...
DLL_32 = 0
DLL_64 = 0
UNIX_BACKEND = $(VENDOR)/libfftw3_32.a
...
ifeq ($(LBITS),64)
	...
	UNIX_BACKEND = $(VENDOR)/libfftw3_64.a
	UNIX_64 = 1
else
...
libbackend:
ifeq ($(DLL_32), 1)
	@cp $(VENDOR)/fftw3.h_32 $(BACK_END)/fftw3.h
	@cp $(VENDOR)/libfftw3-3.dll_32 $(BACK_END)/libfftw3-3.dll
	@cp $(VENDOR)/libfftw3-3.dll_32 $(BACK_END)/libfftw3.dll.a
else
ifeq ($(DLL_64), 1)
	@cp $(VENDOR)/fftw3.h_64 $(BACK_END)/fftw3.h
	@cp $(VENDOR)/libfftw3-3.dll_64 $(BACK_END)/libfftw3-3.dll
	@# This is not a bug. The 32 bit version must be used as the .a
	@cp $(VENDOR)/libfftw3-3.dll_32 $(BACK_END)/libfftw3.dll.a
else
	@cp $(VENDOR)/fftw3.h_ux $(BACK_END)/fftw3.h
endif
endif
...
ifeq ($(DLL_32), 1)
	...
	@cp $(VENDOR)/libiomp5md.dll_32 $(BIN)/libiomp5md.dll
else
ifeq ($(DLL_64), 1)
...
clean_backend: libbackend
	...
	-@[ -e $(BACK_END)/libfftw3-3.dll ] && \
		mv -f $(BACK_END)/libfftw3-3.dll $(BIN)/libfftw3-3.dll
	-@[ -e $(BACK_END)/libbackend.dll ] && \
		mv -f $(BACK_END)/libbackend.dll $(BIN)/libbackend.dll
